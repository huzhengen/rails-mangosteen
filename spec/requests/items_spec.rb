...
RSpec.describe "Items", type: :request do
  describe "获取账目" do
    ...
    it "分页" do
      ...
      get api_v1_items_path, headers: user1.generate_auth_header
      expect(response).to have_http_status(200)
      json = JSON.parse response.body
      expect(json["resources"].size).to eq(10) # default per_page is 10
      get api_v1_items_path, params: { page: 2 }, headers: user1.generate_auth_header
      expect(response).to have_http_status(200)
      json = JSON.parse response.body
    ...
    it "按时间筛选" do
      ...
      get api_v1_items_path, params: { created_after: "2010-1-1", created_before: "2010-1-2" }, headers: user1.generate_auth_header
      expect(response).to have_http_status(200)
      json = JSON.parse response.body
    ...
    it "按时间筛选(只有开始时间)" do
      ...
      get api_v1_items_path, params: { created_after: "2010-01-01" }, headers: user1.generate_auth_header
      expect(response).to have_http_status(200)
      json = JSON.parse response.body
      expect(json["resources"].size).to eq 1
      expect(json["resources"][0]["id"]).to eq item1.id
    ...
    it "按时间筛选(只有结束时间)" do
      ...
      get api_v1_items_path, params: { created_before: "2010-01-05" }, headers: user1.generate_auth_header
      expect(response).to have_http_status(200)
      json = JSON.parse response.body
      expect(json["resources"].size).to eq 1
      expect(json["resources"][0]["id"]).to eq item1.id
